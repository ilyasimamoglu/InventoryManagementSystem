...
namespace InventoryManagementSystem
{
    public partial class OrderForm : Form
    {
        ...
        public void LoadOrder()
        {
            ...
            int i = 0;
            dgvOrder.Rows.Clear();
            cm = new SqlCommand("SELECT orderid, odate, O.pid, P.pname, O.cid, C.cname, qty, price, total  FROM tbtOrder AS O JOIN tbcustomer AS C ON O.cid=C.cid JOIN tbProduct AS P ON O.pid=P.pid WHERE CONCAT ( orderid, odate, O.pid, P.pname, O.cid, C.cname, qty, price) LIKE'%"+txtSearch.Text+"%'", con);
            con.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void dgvUser_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
             ...
             if (colName == "Delete")
            {
                ...
                {
                    con.Open();
                    cm = new SqlCommand("DELETE FROM tbtOrder WHERE orderid LIKE '" + dgvOrder.Rows[e.RowIndex].Cells[1].Value.ToString() + "'", con);
                    cm.ExecuteNonQuery();
                    con.Close();
                    MessageBox.Show("Record has been successfully deleted!");
                    //after delet the order to ad to product the quantity 
                    cm = new SqlCommand("UPDATE tbProduct SET  pqty=(pqty+@pqty)  WHERE pid LIKE '" + dgvOrder.Rows[e.RowIndex].Cells[3].Value.ToString() + "' ", con);
                    cm.Parameters.AddWithValue("@pqty", Convert.ToDouble(dgvOrder.Rows[e.RowIndex].Cells[5].Value.ToString()));
                    con.Open();
                    cm.ExecuteNonQuery();
                    con.Close();
                }
